...
struct fio_option fio_options[FIO_MAX_OPTS] = {
	...
	},
	{
		...
		.lname	= "Rate mode",
		.type	= FIO_OPT_STR,
		.off1	= td_var_offset(rate_mode),
		.help	= "Type of rate mode enforced",
		.def	= "relaxed",
		.category = FIO_OPT_C_IO,
		.group	= FIO_OPT_G_RATE,
		.posval = {
			  { .ival = "relaxed",
			    .oval = RATE_MODE_RELAXED,
			    .help = "Relaxed approach to submit rate",
			  },
			  { .ival = "enforced",
			    .oval = RATE_MODE_ENFORCED,
			    .help = "Enforced submission rate",
			  },
		},
	},
	{
